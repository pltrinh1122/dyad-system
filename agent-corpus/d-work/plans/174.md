...
## Revision — 2026-09-25 (the Operator settles it)
...
One agent-zone PR, branch `agent/174-ui-design-settled`:
1. `agent-corpus/falsification/ui-design-craft.md` — the craft play-book's criteria outcome, the
   measured table of today's rendering duplication, the seven attacks, and the Operator's
   settlement as the verdict, with its `Disposition:` line (Rule-9 Form).
2. One `backlog` row: author `ui-design` as a bolt-on craft for the ASG bundle downstream — assets
   authored upstream, inlined by each projector, with its own guard; not core, not in this repo's
   bundle. `refs` this d-work.
Down from two rows to one; nothing else changes. No craft, no Rule, no projector, no `VERSION`.
