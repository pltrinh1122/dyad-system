...
| `../crafts/sysadmin/` | Rule-11 (a Tended craft, zone `craft`); its rules Tended | the system-administration craft: rules, vocabulary, templates, guards with tests, one projector (`project_events.py`), docs, records; extracted by #155, its reference git-server deployment moved out to its own craft, `crafts/lan-git/` (`requires: sysadmin>=0.1.0`), by #181; authored here since #58 (#57's audit), authoring moved from `pltrinh1122/workstation` | no |
