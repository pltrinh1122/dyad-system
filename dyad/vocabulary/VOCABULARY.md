...
| release | a git tag marking a craft's published version with its VERSION file and exported archive — `vMAJOR.MINOR.PATCH` for the core craft, `<craft>-vMAJOR.MINOR.PATCH` for a Tended craft; cut only on the Operator's `Y` to `Y/N: release <tag>?` | 11 | 11 |
